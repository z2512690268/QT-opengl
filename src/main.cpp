...
class OpenGLESTest : public QWindow {
    public:
        OpenGLESTest();
...
void
OpenGLESTest::timerEvent(QTimerEvent *event)
{
    if (!isExposed()) {
    ...
    foreach (float reading, readings) {
        ...
        float fgvtx[] = {
            x + border, y + border,
            x + border + (width - 2 * border) * reading /(float) 9.81, y + border,
            x + border, y + height - border,
            x + border + (width - 2 * border) * reading / (float)9.81, y + height - border,
        };
...
void
OpenGLESTest::touchEvent(QTouchEvent *event)
{
...
bool
OpenGLESTest::event(QEvent *event)
{
    switch (event->type()) {
        case QEvent::Show:
            if (initial) {
